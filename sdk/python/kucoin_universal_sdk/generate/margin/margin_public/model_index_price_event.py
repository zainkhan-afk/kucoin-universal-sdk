...
class IndexPriceEvent(BaseModel):
    """
    IndexPriceEvent
    ...
    @classmethod
    def from_json(cls, json_str: str) -> Optional[IndexPriceEvent]:
...
IndexPriceEventCallback = Callable[[str, str, IndexPriceEvent], None]
"""
args:
    - topic (str) : topic
    - subject (str): subject
    - data (IndexPriceEvent): event data
"""


class IndexPriceEventCallbackWrapper(WebSocketMessageCallback):

    def __init__(self, cb: IndexPriceEventCallback):
        self.callback = cb

    def on_message(self, message: WsMessage):
        event = IndexPriceEvent.from_dict(message.raw_data)
